...
type PermissionManagerCb interface {
	OnAccessDenied(s *AccessMetadata) error
	OnAccessGranted(s *AccessMetadata)
}
...
func (pm *PermissionsManager) OnAccess(key string, matches permMatches, resType ResourceType, req Access) error {
	...
	/*
		ambientPolicy: allow
		depsPolicy: allow
		-> here we just need to walk the AccessMetadata and find removed permissions.
		...
		AccessMetadata for removed / intersection of permissions.
	...
	*/
	...
	// Need to always walk the AccessMetadata, since the calling dep could be part of a callback.
	...
	if !dangerousAllowed && !allowed {
		return pm.cb.OnAccessDenied(s)
	}

	pm.cb.OnAccessGranted(s)
